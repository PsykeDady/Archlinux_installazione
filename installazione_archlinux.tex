...
\definecolor{lred}{rgb}{1,0.3,0.3} %come fare un colore
\definecolor{codeback}{rgb}{0,0,0}
\definecolor{codefore}{rgb}{0,255,0}

\lstdefinestyle{mystyle}{
	backgroundcolor=\color{codeback},
	basicstyle=\ttfamily\color{codefore}
}
...
\section{Metodo 2  da Linux:  copia su fat32 (Consigliato UEFI)}
    ...
    e montate quindi la iso utilizzando: \centcode{sudo mount -o loop </percorso/iso> iso}
    ...
    \centcode{dd if=/dev/zero of=/dev/sdX \nlcode
        \# \textit{da eseguire solo se si vuole completamente resettare la pennina} \nlcode
        fdisk /dev/sdX \\
        \\
        \# \textit{si entrerà in modalità fdisk, scrivere i prossimi comandi e premere invio dopo ciascuno}
        o\\
        p\\
        1\\
        2048\\
        \# \textit{invio senza scrivere nulla oppure scegliere una dimensione}\\
        t\\
        b\\
        w \# \textit{ora si uscirà dalla modalità fdisk}\\
        \nlcode
        mkfs.fat /devX1 \nlcode
        mount /dev/sdX1 usb \nlcode
        dosfslabel /dev/sdX1 ARCH\_AAAAMM \nlcode
        \# \textit{sostituendo ad AAAA ed MM le stesse date che trovate sulla iso}}
...
\section{Connessione al mondo esterno e installazione dei primi pacchetti}
    ...
    Se quindi siete connessi via cavo, basta dare \code{dhcpcd}, altrimenti archlinux fornisce una comodissima interfaccia di rete wireless, a cui potete accedere con: \centcode{wifi-menu} 
    Scegliete quindi il vostro SSID di fiducia, scrivete la password (se ne avete una) e date \code{dhcpcd} per forzare il router a rilasciarvi un indirizzo IP. Ben fatto, siete connessi! Sicuri? Per accertarcene possiamo dare \centcode{ping -c 3 www.google.com} e, se la connessione effettivamente c'è, otterrete in output l'informazione che son stati trasmessi e ricevuti 3 pacchetti. Ora che siamo sicuri possiamo andare avanti.\\
...
\section{Prime configurazioni}
    ...
    A questo punto configuriamo il grub: \centcode{grub-mkconfig -o /boot/grub/grub.cfg \nlcode grub-install /dev/sda}
